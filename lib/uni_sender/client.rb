...
module UniSender
  ...
  private
    ...
    def default_request(action, params={})
      params = translate_params(params).delete_if { |_, v| v.empty? } if defined?('translate_params')
      params.merge!({ 'api_key' => api_key, 'format' => 'json' })
      response = Net::HTTP.post_form(URI("http://apis.unisender.com/#{locale}/api/#{action}"), params)
